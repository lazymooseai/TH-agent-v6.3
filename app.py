...
# Päivämäärävalinta: tänään tai huominen
...
.badge-orange { background:#5a2a00; color:#ffb347; padding:2px 8px; border-radius:4px; font-size:16px; font-weight:bold; }
...
# ── KORJATUT URHEILU- JA KULTTUURI-SKRAPPAAJAT (PARAMETRISET) ────────────────

# 

# Kaikki funktiot ottavat nyt pvm_iso-parametrin (“YYYY-MM-DD”),

# joten samaa funktiota voi käyttää sekä tänään että huomenna.

# Välimuistiavain sisältää päivämäärän → päivät eivät sekoitu.

def _pvm_muodot(pvm_iso: str):
“”“Palauttaa yleisimmät päivämääräformaatit hakua varten.”””
...
“””
Yleinen apufunktio: etsii päivämäärän ympäriltä näytösotsikkoja ja aikoja.
Kulkee HTML-puussa ylöspäin löytääkseen lähimmän otsikkoelementin.
“””
tapahtumat = []
seen = set()
...
for pvm in pvm_muodot:
    # Etsi kaikki tekstisolmut joissa päivämäärä esiintyy
    for el in soup.find_all(string=re.compile(re.escape(pvm))):
        node = el.parent
        for _ in range(10):  # Kävele ylöspäin max 10 tasoa
            if node is None:
                break
            # Etsi otsikko tästä kontainerista
            heading = node.find(["h1", "h2", "h3", "h4", "strong"])
            if heading:
                title = heading.get_text(strip=True)
                # Ohita lyhyet, numeeriset tai jo löydetyt otsikot
...
“””
Hakee SM-Liigan kotipelit annetulle päivälle.
Korjaukset v6.3: oikea season-muoto (YYYY-YYYY), useita endpoint-vaihtoehtoja,
joustava joukkuenimen tunnistus.
“””
try:
dt = datetime.datetime.strptime(pvm_iso, “%Y-%m-%d”)
kausi_alku = dt.year if dt.month > 6 else dt.year - 1
kausi_str = f”{kausi_alku}-{kausi_alku + 1}”   # ← Korjaus: “2025-2026” eikä “2025”

```
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}

    # Kokeillaan useita endpoint-vaihtoehtoja järjestyksessä
    endpoints = [
    ...
    for url in endpoints:
        try:
            ...
            if resp.status_code != 200:
                continue
            data = resp.json()
            # API voi palauttaa listan tai dictin (data.games / data.data)
            pelit_lista = (
                data if isinstance(data, list)
                else data.get("games", data.get("data", []))
            )
            ...
            for peli in pelit_lista:
                ...
                # Joustava joukkuenimen haku – eri API-versiot käyttävät eri kenttiä
                koti = (
                    (peli.get("homeTeam") or {}).get("teamName")
                    or (peli.get("homeTeam") or {}).get("name")
                    or peli.get("homeTeamName", "")
                )
                vieras = (
                    (peli.get("awayTeam") or {}).get("teamName")
                    or (peli.get("awayTeam") or {}).get("name")
                    or peli.get("awayTeamName", "")
                )
                aika = start[11:16] if len(start) > 10 else "-"
                pelit.append({"koti": koti, "vieras": vieras, "aika": aika})

            if pelit or len(pelit_lista) > 0:
                # API vastasi – palautetaan tulos vaikka pelejä ei olisi
                return pelit
...
“””
Hakee Kansallisoopperan näytökset annetulle päivälle.
Parsii ohjelmistosivulta otsikot ja ajat HTML-rakenteesta.
Fallback: tekstihaku palauttaa pelkän ilmoituksen.
“””
try:
pvm_muodot = _pvm_muodot(pvm_iso)
headers = {
...
resp = requests.get(“https://oopperabaletti.fi/ohjelmisto/”, headers=headers, timeout=12)
resp.raise_for_status()
soup = BeautifulSoup(resp.text, “html.parser”)
...
```
    ...
    # Fallback: jos rakenteellinen haku ei tuottanut tuloksia,
    # kokeillaan löytyykö päivämäärä tekstistä ylipäänsä
    if not tapahtumat:
        full_html = resp.text
        pvm_fi = pvm_muodot[0]
        for pvm in pvm_muodot:
            if pvm in full_html:
                # Yritetään poimia konteksti regex-haulla
                ...
                if match:
                    ...
                    # Etsitään esityksen nimi kontekstista
                    time_match = re.search(r"\b([0-2]?\d:[0-5]\d)\b", context)
                    aika_str = f" klo {time_match.group(1)}" if time_match else ""
                    tapahtumat = [f"Näytös {pvm_fi}{aika_str} (tarkista oopperabaletti.fi)"]
                else:
...
“””
Hakee Helsingin Kaupunginteatterin näytökset annetulle päivälle.
Parsii kalenteri-sivulta näytösotsikot, ajat ja näyttämöt.
“””
try:
pvm_muodot = _pvm_muodot(pvm_iso)
headers = {
...
```
    ...
    for url in ["https://hkt.fi/kalenteri/", "https://hkt.fi/"]:
        try:
            ...
            # Fallback: tarkista löytyykö päivämäärä ylipäänsä
            for pvm in pvm_muodot:
                if pvm in resp.text:
                    pvm_fi = pvm_muodot[0]
                    # Poimi konteksti regex-haulla
                    raw = resp.text
                    ...
                    for m in matches[:3]:
                        ...
                        # Etsi isolla kirjoitettu nimi (HKT käyttää suuraakkosia)
...
“”“Hakee LinkedEvents-API:sta kulttuuritapahtumat annetulle päivälle.”””
live_tapahtumat = {}
try:
...
url = f”https://linkedevents.api.hel.fi/v1/event/?start={alku}&end={loppu}&include=location&limit=200”
    ...
    for e in resp.json().get("data", []):
        ...
        try:
            ...
            if end_dt.date() == dt.date():
                ...
                live_tapahtumat[paikka_nimi].append(
                    f"{e.get('name', {}).get('fi', 'Tuntematon esitys')} ({end_dt.strftime('%H:%M')})"
                )
        except:
            pass
except:
...
“””
Yhdistää kulttuuripaikkojen tapahtumatiedot kolmesta lähteestä:
LinkedEvents API, HKT-skrapperi ja Ooppera-skrapperi.
“””
...
for p in paikat:
    ...
    # Lähde 2: Paikan omat skrapparit (HKT ja Ooppera)
    ...
    if tapahtumat:
        p["lopetus_html"] = (
            f"<span class='live-event'>🟢 ESITYKSIÄ: {' | '.join(tapahtumat)}</span>"
        )
    else:
        if hakusanat:
            p["lopetus_html"] = (
                f"<span class='no-event'>⚪ Ei tunnettua esitystä.</span>"
                f"<br><span style='color:#777;'>Tyypillisesti: {p['lopetus']}</span>"
            )
        else:
            p["lopetus_html"] = f"<span class='endtime'>⏱ Tyypillinen lopetus: {p['lopetus']}</span>"
return paikat
```

def yhdista_urheiludata(paikat, pvm_iso: str):
“””
Hakee Liiga-pelit annetulle päivälle ja yhdistää ne urheilupaikkatietoihin.
“””
liiga_pelit = hae_liiga_pvm(pvm_iso)

```
# Joustava joukkuehaku: tarkistaa sekä koti- että vierasjoukkueen nimen
def etsi_kotipeli(hakusana):
    ...
    for peli in liiga_pelit:
        ...
        if hakusana.lower() in koti_lower:
            tulokset.append(
                f"{peli['koti']} – {peli['vieras']} (klo {peli['aika']})"
            )
    return tulokset
...
for p in paikat:
    ...
    if tapahtumat:
        p["lopetus_html"] = (
            f"<span class='live-event'>🏒 PELI TÄNÄÄN: {' | '.join(tapahtumat)}</span>"
            f"<br><span style='color:#ccc;font-size:15px;'>Kesto n. 2,5h aloituksesta</span>"
        )
    else:
...
“”“Hakee LinkedEvents-API:sta yleiset tapahtumat annetulle päivälle.”””
tulos = []
try:
dt = datetime.datetime.strptime(pvm_iso, “%Y-%m-%d”).replace(tzinfo=ZoneInfo(“Europe/Helsinki”))
now = datetime.datetime.now(ZoneInfo(“Europe/Helsinki”))
resp = requests.get(
f”https://linkedevents.api.hel.fi/v1/event/?start={pvm_iso}&sort=end_time&include=location”,
timeout=10
)
resp.raise_for_status()
for e in resp.json().get(“data”, []):
...
osoite = (
loc.get(“name”, {}).get(“fi”)
or loc.get(“street_address”, {}).get(“fi”)
or “Helsinki”
)
tulos.append({
...
f”<span class='venue-name'>{p[‘nimi’]}</span><br>”
f”<span class='venue-address'>Max pax: <b>{p[‘kap’]}</b></span><br>”
f”<span style='color:#ccc;font-size:17px;'>{p[‘huomio’]}</span><br>”
f”{lopetus_naytto}<br>”
f”<a href=’{p[‘linkki’]}’ class=‘taksi-link’ target=’_blank’ style=‘font-size:16px;’>{p[‘teksti’]} →</a>”
)
if “linkki2” in p:
card_html += f”   |   <a href=’{p[‘linkki2’]}’ class=‘taksi-link’ target=’_blank’ style=‘font-size:16px;’>{p[‘teksti2’]} →</a>”
...
if junat and junat[0]["train"] != "API-virhe":
    for j in junat:
        juna_html += (
            ...
            f"<span style='color:#aaa;'>(lähtö: {j['origin']}"
            f"{'  ⭐' if j['origin'] in {'Rovaniemi', 'Kolari', 'Kemi', 'Oulu', 'Kajaani', 'Kuopio', 'Joensuu', 'Iisalmi', 'Ylivieska'} else ''})</span><br>"
...
st.markdown(
    ...
    f"<a href='{vr_linkit[valittu]}' class='taksi-link' target='_blank'>VR Live ({valittu}) &#x2192;</a>"
    f" &nbsp;&nbsp; <a href='https://www.vr.fi/radalla/poikkeustilanteet' class='taksi-link' target='_blank' style='color:#ff9999;'>⚠ VR Poikkeukset &#x2192;</a></div>",
    unsafe_allow_html=True,
)
...
with col_a:
    averio_html = (
        "<div class='card-title'>Averio – Matkustajamäärät</div>"
        "<span style='color:#aaa;font-size:15px;'>⚠ klo 00:30 MS Finlandia → <b>Länsisatama T2</b>, ei Vuosaari</span><br><br>"
    )
...
with col_b:
    ...
    st.markdown(
        ...
        f"<a href='https://www.portofhelsinki.fi/matkustajille/matkustajatietoa/lahtevat-ja-saapuvat-matkustajalaivat/#tabs-2' class='taksi-link' target='_blank'>Helsingin Satama →</a></div>",
        unsafe_allow_html=True,
    )
...
if lento_virhe:
    st.markdown(
        f"<div class='taksi-card'><div class='card-title'>Finavia API</div>"
        f"<span style='color:#ff9999;'>⚠ {lento_virhe}</span><br><br>"
        f"<a href='https://www.finavia.fi/fi/lentoasemat/helsinki-vantaa/lennot?tab=arr' class='taksi-link' target='_blank'>Finavia – Saapuvat lennot →</a></div>",
        unsafe_allow_html=True,
    )
else:
    lento_html = (
        "<div class='card-title'>Taktiset poiminnat – saapuvat</div>"
        "<span style='color:#aaa;font-size:15px;'>Frankfurt arki-iltaisin = paras business-lento | Sähköautoilla tolppaetuoikeus</span><br><br>"
    )
    for lento in lennot:
        lento_html += (
            f"<b>{lento['time']}</b>  ·  {lento['origin']} <span style='color:#aaa;'>({lento['flight']})</span><br>"
            f"  └ <span class='{'pax-good' if lento['wb'] else 'pax-ok'}'>{lento['type']}</span> | {lento['status']}<br>"
            f"  └ {laske_kysyntakerroin(lento['wb'], lento['time'])}<br><br>"
        )
    st.markdown(
        f"<div class='taksi-card'>{lento_html}"
        f"<a href='https://www.finavia.fi/fi/lentoasemat/helsinki-vantaa/lennot?tab=arr' class='taksi-link' target='_blank'>Finavia Saapuvat →</a></div>",
        unsafe_allow_html=True,
    )
...
# ── PÄIVÄMÄÄRÄVALINTA: TÄNÄÄN / HUOMENNA ─────────────────────────────────
...
with tab1:
    kulttuuri_paikat = [
        {"nimi": "Helsingin Kaupunginteatteri (HKT)", "kap": "947 hlö", "hakusanat": ["kaupunginteatteri", "hkt"], "huomio": "Paras yksittäinen kohde. Eläintarha.", "lopetus": "klo 21:30–22:30", "linkki": "https://hkt.fi/kalenteri/", "teksti": "HKT Kalenteri", "badge": "badge-red"},
        {"nimi": "Kansallisooppera ja -baletti", "kap": "1 700 hlö", "hakusanat": ["ooppera", "baletti"], "huomio": "Ooppera = parhaat kyydit. Töölönlahti.", "lopetus": "klo 21:00–23:00", "linkki": "https://oopperabaletti.fi/kalenteri/", "teksti": "Ooppera Ohjelmisto", "badge": "badge-yellow"},
        {"nimi": "Kansallisteatteri", "kap": "1 000 hlö", "hakusanat": ["kansallisteatteri"], "huomio": "Aseman vieressä, Rautatientori.", "lopetus": "klo 21:00–22:30", "linkki": "https://www.kansallisteatteri.fi/ohjelmisto/ohjelmistokalenteri", "teksti": "Kansallisteatteri Kalenteri", "badge": "badge-blue"},
        {"nimi": "Musiikkitalo", "kap": "1 704 hlö", "hakusanat": ["musiikkitalo"], "huomio": "Mannerheimintie.", "lopetus": "klo 21:00–22:30", "linkki": "https://musiikkitalo.fi/tapahtumat/", "teksti": "Musiikkitalo Tapahtumat", "badge": "badge-blue"},
        {"nimi": "Tanssin talo (Kaapelitehdas)", "kap": "1 000 hlö", "hakusanat": ["tanssin talo", "kaapelitehdas"], "huomio": "Modernin tanssin keskus. Erkko-sali 700 hlö, Pannuhalli 235 hlö.", "lopetus": "klo 21:00–22:30", "linkki": "https://www.tanssintalo.fi/ohjelma", "teksti": "Tanssin talo Kalenteri", "badge": "badge-blue"},
        {"nimi": "Helsingin Suomalainen Klubi", "kap": "300 hlö", "hakusanat": [], "huomio": "Yritysjohto – pitkät iltakyydit arki-iltaisin. Kamppi.", "lopetus": "klo 22:00–01:00", "linkki": "https://tapahtumat.klubi.fi/tapahtumat/", "teksti": "Klubi Tapahtumat", "badge": "badge-red"},
        {"nimi": "Svenska Klubben", "kap": "200 hlö", "hakusanat": [], "huomio": "Korkeaprofiilinen – erityisseuranta tapahtumailtaisin. Kruununhaka.", "lopetus": "klo 22:00–01:00", "linkki": "https://klubben.fi/start/program/", "teksti": "Svenska Klubben Ohjelma", "badge": "badge-red"},
        {"nimi": "Finlandia-talo", "kap": "1 700 hlö", "hakusanat": ["finlandia-talo", "pikku-finlandia"], "huomio": "Kongressit ja gaalat. Hyvä yrityspoistumat iltaisin.", "lopetus": "klo 21:00–23:00", "linkki": "https://finlandiatalo.fi/finlandia-talo/tulevat-tapahtumat/", "teksti": "Finlandia-talo Kalenteri", "badge": "badge-yellow"},
    ]
    st.markdown(f"<div class='taksi-card'>{venue_html(yhdista_kulttuuridata(kulttuuri_paikat, pvm_iso))}</div>", unsafe_allow_html=True)

with tab2:
    urheilu_paikat = [
        {"nimi": "HIFK – Nordis (jääkiekko)", "kap": "8 200 hlö", "huomio": "Paras seura kyytien kannalta. Poistumapiikki ~2,5 h kiekon jälkeen.", "lopetus": "klo 21:30–23:00 (60+60+20 min + mahdolliset jatkoajat)", "linkki": "https://liiga.fi/fi/ohjelma?kausi=2025-2026&sarja=runkosarja&joukkue=hifk&kotiVieras=koti", "teksti": "HIFK Kotiottelut", "badge": "badge-red"},
        {"nimi": "Kiekko-Espoo – Metro Areena", "kap": "8 500 hlö", "huomio": "Suurin areena. Kyydit myös Espoosta Helsinkiin.", "lopetus": "klo 21:30–23:00", "linkki": "https://liiga.fi/fi/ohjelma?kausi=2025-2026&sarja=runkosarja&joukkue=k-espoo&kotiVieras=koti", "teksti": "Kiekko-Espoo Kotiottelut", "badge": "badge-yellow"},
        {"nimi": "Veikkaus Arena (Jokerit & Tapahtumat)", "kap": "15 000 hlö", "huomio": "Jokereiden kotiottelut ja suurtapahtumat. Valtava poistumapiikki.", "lopetus": "klo 21:00–22:30 (jääkiekko), 23:00 (konsertit)", "linkki": "https://www.veikkausarena.fi/fi/all-events", "teksti": "Veikkaus Arena Tapahtumat", "badge": "badge-red"},
        {"nimi": "Bolt Arena (HJK)", "kap": "10 770 hlö", "huomio": "Veikkausliiga ja HJK kotiottelut. Töölö. Huono sää nostaa kyytikerrointa.", "lopetus": "klo 21:00–22:30 (45+45+lisäaika)", "linkki": "https://www.hjk.fi/ottelut/miehet/", "teksti": "HJK Otteluohjelma", "badge": "badge-blue"},
        {"nimi": "Olympiastadion (jalkapallo / tapahtumat)", "kap": "50 000 hlö", "huomio": "Maaottelut, konsertit. Suuri poistumapiikki.", "lopetus": "klo 21:30–23:30 (konserteilla myöhemmin)", "linkki": "https://olympiastadion.fi/tapahtumat", "teksti": "Olympiastadion Tapahtumat", "badge": "badge-red"},
    ]
    st.markdown(f"<div class='taksi-card'>{venue_html(yhdista_urheiludata(urheilu_paikat, pvm_iso))}</div>", unsafe_allow_html=True)

with tab3:
    messut_paikat = [
        {"nimi": "Messukeskus", "kap": "50 000 hlö", "huomio": "Poistumapiikki oviensulkemisaikaan – EI alkamisaikaan! Pasila.", "lopetus": "Sulkeutuu klo 17:00–18:00 (messut) tai 23:00 (konsertit)", "linkki": "https://messukeskus.com/kavijalle/tapahtumat/tapahtumakalenteri/", "teksti": "Messukeskus Kalenteri", "badge": "badge-red"},
        {"nimi": "Aalto-yliopisto / Dipoli (Espoo)", "kap": "1 000 hlö", "huomio": "Kansainväliset kongressit. Business-asiakkaat, pitkät kyydit.", "lopetus": "klo 17:00–21:00", "linkki": "https://www.dipoli.fi/fi/", "teksti": "Dipoli Info", "badge": "badge-yellow"},
        {"nimi": "Kalastajatorppa / Pyöreä Sali", "kap": "500 hlö", "huomio": "Business-illalliset. Pitkät kyydit kantakaupunkiin.", "lopetus": "klo 22:00–00:00", "linkki": "https://kalastajatorppa.fi/tapahtumat/", "teksti": "Kalastajatorppa Tapahtumat", "badge": "badge-yellow"},
        {"nimi": "Helsingin alueen yleiskalenterit", "kap": "Koko kaupunki", "huomio": "Paras yleissilmäys kaikista kaupungin tapahtumista.", "lopetus": "Vaihtelee", "linkki": "https://stadissa.fi/", "teksti": "Stadissa.fi", "linkki2": "https://www.myhelsinki.fi/fi/tapahtumat", "teksti2": "MyHelsinki Tapahtumat", "badge": "badge-blue"},
    ]
    st.markdown(f"<div class='taksi-card'>{venue_html(messut_paikat)}</div>", unsafe_allow_html=True)

with tab4:
    musiikki_paikat = [
        {"nimi": "Tavastia", "kap": "900 hlö", "huomio": "Paras keikkapaikka kyytien kannalta.", "lopetus": "klo 22:30–01:00", "linkki": "https://tavastiaklubi.fi/", "teksti": "Tavastia Ohjelma", "badge": "badge-yellow"},
        {"nimi": "On the Rocks", "kap": "600 hlö", "huomio": "Rock, metal. Tarkista täyttöaste ennen siirtymistä.", "lopetus": "klo 22:00–01:00", "linkki": "https://www.rocks.fi/tapahtumat/", "teksti": "On the Rocks Ohjelma", "badge": "badge-yellow"},
        {"nimi": "Kulttuuritalo", "kap": "1 500 hlö", "huomio": "Isommat rock-konsertit. Hyvä kyytipotentiaali.", "lopetus": "klo 22:00–00:00", "linkki": "https://kulttuuritalo.fi/tapahtumat/", "teksti": "Kulttuuritalo Tapahtumat", "badge": "badge-yellow"},
        {"nimi": "Malmitalo", "kap": "400 hlö", "huomio": "Iskelmä, kansanmusiikki. Kauempana – hyvä päivävuoroille.", "lopetus": "klo 21:00–22:30", "linkki": "https://www.malmitalo.fi/", "teksti": "Malmitalo Tapahtumat", "badge": "badge-blue"},
        {"nimi": "Sellosali (Espoo)", "kap": "400 hlö", "huomio": "Klassinen, jazz. Pitkä kyyti kantakaupunkiin.", "lopetus": "klo 21:00–22:00", "linkki": "https://www.espoo.fi/fi/sellosalin-tapahtumat", "teksti": "Sellosali Ohjelma", "badge": "badge-blue"},
        {"nimi": "Flow Festival / Suvilahdentie", "kap": "30 000 hlö / päivä", "huomio": "Kesäfestivaalit (elokuu). Erittäin suuri poistumapiikki illalla.", "lopetus": "klo 23:00–01:00", "linkki": "https://www.flowfestival.com/ohjelma/", "teksti": "Flow Festival Ohjelma", "badge": "badge-red"},
    ]
...
with tab5:
    ...
    if general_tapahtumat:
        ...
        st.markdown("<div class='taksi-card'>Ei muita tiedossa olevia yleisötapahtumia valitulle päivälle.</div>", unsafe_allow_html=True)
...
st.markdown(
    ...
    "TH Taktinen Tutka v6.3 – Päivittyy 5 min välein – Digitraffic · Port of Helsinki · Averio · Finavia API · LinkedEvents"
    "</div>",
    unsafe_allow_html=True,
)
```
